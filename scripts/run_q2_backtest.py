...
not an auction-vs-continuous spread.

Kept runnable for the audit trail only. Its shipped artifacts live in
``results/deprecated_q2_backtest_hourly_marks/`` (see the DEPRECATED.md
there); do not quote its Sharpe ratios.

Usage:
    .venv/bin/python scripts/run_q2_backtest.py [--end YYYY-MM-DD | --end live]
"""
...
# when another ``scripts`` package shadows this one on sys.path).
_spec_file = Path(__file__).resolve().parent / "q2_strategy_spec.py"
_spec = importlib.util.spec_from_file_location("q2_strategy_spec", _spec_file)
_q2_spec = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_q2_spec)

COST_SCENARIOS = _q2_spec.COST_SCENARIOS
ENTRY_Z = _q2_spec.ENTRY_Z
EXIT_Z = _q2_spec.EXIT_Z
MAX_HOLD = _q2_spec.MAX_HOLD
N_BOOTSTRAP = _q2_spec.N_BOOTSTRAP
SEED = _q2_spec.SEED
STOP_LOSS_Z = _q2_spec.STOP_LOSS_Z
TEST_MONTHS = _q2_spec.TEST_MONTHS
TRAIN_MONTHS = _q2_spec.TRAIN_MONTHS

OUT = ROOT / "results" / "deprecated_q2_backtest_hourly_marks"
FIG = OUT / "figures"
TAB = OUT / "tables"
...
def main() -> None:
    ...
    for scen, res in results.items():
        ...
        for y, v in yr.items():
            pnl_by_year_rows.append(
                {"scenario": scen, "year": int(y), "pnl_eur_per_mwh": round(float(v), 2)}
            )
    ...
    ax.set_title(
        f"Block bootstrap Sharpe distribution (realistic costs, p = {real.bootstrap_pvalue:.3f})"
    )
    ax.legend()
